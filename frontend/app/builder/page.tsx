...
export default function BuilderPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(e.message || "Failed to generate strategy. Is the backend running?");
    } finally {
      setLoading(false);
    }
  };
  ...
  const useTemplate = (t: StrategyTemplate) => {
    setCode(t.code);
            ...
            {activeTab === "describe" ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                {templates.length === 0 ? (
                  ...
                  templates.map(t => (
                    <div key={t.id} className="card card-interactive" style={{ cursor: "pointer" }} onClick={() => useTemplate(t)}>
